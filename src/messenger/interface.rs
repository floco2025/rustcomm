...
///
/// Allows multiple threads to send messages to the same [`Messenger`]. Obtain
/// an instance by calling [`Messenger::get_messenger_interface()`].
///
...
impl<C: Context> MessengerInterface<C> {
    ...
    ///
    /// This is thread-safe and blocks until the listener is created. The
    /// Transport's event loop will process the request and return the listener
    /// ID and actual bound address.
    ///
    /// **Note:** If thread-safety is not required, call [`Messenger::listen()`]
    ...
    ///
    /// This is thread-safe and blocks until the connection is established. The
    /// Transport's event loop will process the request and return the
    /// connection ID and peer's socket address.
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::connect()`] directly for better performance.
    ...
    ///
    /// This is thread-safe and blocks until the addresses are retrieved. The
    /// Transport's event loop will process the request and return the
    /// addresses.
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::get_listener_addresses()`] directly for better performance.
    ...
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::send_to_with_context()`] directly for better performance.
    ...
    ///
    /// Calls [`send_to_many_with_context()`] with an empty context.
    ...
    /// Queues a message with context to be sent to multiple specific
    /// connections.
    ///
    /// This is thread-safe and non-blocking. The message will be serialized and
    /// sent when the Transport's event loop processes it.
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::send_to_many_with_context()`] directly for better
    /// performance.
    ...
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::broadcast_with_context()`] directly for better performance.
    ...
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::broadcast_except_with_context()`] directly for better
    /// performance.
    ...
    /// Queues a message with context to be broadcast to all connected clients
    /// except multiple specified ones.
    ///
    /// This is thread-safe and non-blocking. The message will be serialized and
    /// sent when the Transport's event loop processes it.
    ///
    /// **Note:** If thread-safety is not required, call
    /// [`Messenger::broadcast_except_many_with_context()`] directly for better
    /// performance.
