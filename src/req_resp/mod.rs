...
use crate::{Message, Messenger, MessengerInterface, MessengerEvent};
...
impl RequestResponse {
    ...
    /// 
    /// Returns `Box<dyn Message>` which the caller must downcast to the expected response type.
    /// 
    /// The request message must have `get_request_id()` and `set_request_id()` implemented.
    /// The request ID will be set automatically before sending.
    pub async fn send_request(&self, peer_id: usize, mut request: Box<dyn Message>) -> Box<dyn Message> {
        ...
        rx.await.expect("Event loop dropped sender or connection closed")
    }
