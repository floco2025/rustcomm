...
// ============================================================================
// TLS Configuration Loading Helpers
// ============================================================================
...
fn load_tls_server_config(cert_path: &str, key_path: &str) -> Result<rustls::ServerConfig, Error> {
    ...
    let cert_chain: Vec<rustls::pki_types::CertificateDer> = certs(&mut BufReader::new(cert_file))
        ...
        .map_err(|e| {
            Error::TlsInvalidCertificate(format!("Failed to parse certificates: {}", e))
        })?;
...
// Loads TLS client configuration from CA certificate file.
//
// # Arguments
// * `ca_cert_path` - Path to the PEM-encoded CA certificate file
fn load_tls_client_config(ca_cert_path: &str) -> Result<rustls::ClientConfig, Error> {
    ...
    let ca_certs: Vec<rustls::pki_types::CertificateDer> = certs(&mut BufReader::new(ca_cert_file))
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| {
